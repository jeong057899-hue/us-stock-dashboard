...
# =========================================================
# 상단 설정 패널: 사이드바에 의존하지 않음
# =========================================================
with st.expander("⚙️ 설정 옵션 열기 / 닫기", expanded=False):
    set_col1, set_col2, set_col3, set_col4, set_col5 = st.columns([1.15, 1, 1, 1, 1.2])

    with set_col1:
        theme_mode = st.selectbox("화면 테마", ["다크 모드", "라이트 모드"], index=0)

    with set_col2:
        auto_refresh = st.checkbox("자동 갱신", value=False)

    with set_col3:
        refresh_seconds = st.selectbox("갱신 주기", [15, 30, 60, 120, 300], index=2)

    with set_col4:
        chart_period = st.selectbox("차트 기간", ["1d", "5d", "1mo", "3mo", "6mo", "1y"], index=0)

    with set_col5:
        chart_interval = st.selectbox("차트 간격", ["1m", "5m", "15m", "30m", "1h", "1d"], index=1)

    set_col6, set_col7 = st.columns([1.4, 2.6])
    ...
    for name, symbol in ticker_dict.items():
        try:
            ...
            if len(hist) >= 2:
                ...
                rows.append({
                    "이름": name, "티커": symbol, "현재가": None,
                    "변동률(%)": None, "거래량": None, "평균거래량대비": None,
                })
    ...
    for idx, symbol in enumerate(list(st.session_state.focus_symbols)):
        ...
        with col:
            if data:
                ...
                st.markdown('<div class="focus-delete-row">', unsafe_allow_html=True)
