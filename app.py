...
theme_mode = st.sidebar.selectbox("화면 테마", ["다크 모드", "라이트 모드"], index=0)
...
[data-testid="stHeader"]::before {{ background: transparent !important; }}
.block-container {{ padding-top: 0.35rem !important; padding-left: 1.6rem; padding-right: 1.6rem; max-width: 98.5vw !important; }}
...
[data-testid="stMetric"] {{ background: {PANEL_BG}; border: 1px solid {BORDER}; padding: 15px 17px; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.20); min-height: 105px; }}
[data-testid="stMetricLabel"] {{ color: {TEXT_SUB}; font-size: 0.80rem; }}
[data-testid="stMetricValue"] {{ color: {TEXT_MAIN}; font-size: 1.50rem; font-weight: 750; }}
[data-testid="stMetricDelta"] {{ font-size: 0.80rem; }}
.panel {{ background: {PANEL_BG}; border: 1px solid {BORDER}; border-radius: 18px; padding: 16px; box-shadow: 0 10px 26px rgba(0,0,0,0.20); margin-bottom: 14px; color: {TEXT_MAIN}; }}
.panel-title {{ font-size: 1.08rem; font-weight: 800; margin-bottom: 12px; color: {TEXT_MAIN}; }}
.status-pill {{ display: inline-flex; align-items: center; padding: 7px 16px; border-radius: 999px; background: rgba(34,197,94,0.10); border: 1px solid rgba(34,197,94,0.35); color: #22c55e; font-weight: 800; font-size: 1.05rem; }}
.news-card {{ background: {PANEL_BG}; border: 1px solid {BORDER}; border-radius: 15px; padding: 14px; min-height: 132px; max-height: 132px; overflow: hidden; margin-bottom: 10px; }}
.news-card:hover {{ transform: translateY(-2px); transition: 0.2s; border-color: #38bdf8; }}
.news-meta {{ font-size: 0.74rem; color: {TEXT_SUB}; margin-bottom: 8px; }}
.news-title {{ font-size: 0.88rem; line-height: 1.35; font-weight: 700; }}
...
.news-link {{ font-size: 0.8rem; color: #38bdf8; margin-top: 8px; }}
.badge-green {{ color: #22c55e; font-weight: 800; }}
.badge-red {{ color: #ef4444; font-weight: 800; }}
.badge-yellow {{ color: #f59e0b; font-weight: 800; }}
.comment-title {{ font-weight: 800; font-size: 0.92rem; color: {TEXT_MAIN}; }}
.comment-desc {{ font-size: 0.82rem; color: {TEXT_SUB}; line-height: 1.4; margin-top: 5px; }}
...
.focus-card {{ background: {PANEL_BG}; border: 1px solid {BORDER}; border-radius: 18px; padding: 14px 16px; margin-bottom: 8px; min-height: 118px; box-shadow: 0 10px 24px rgba(0,0,0,0.18); }}
.focus-name {{ font-weight: 850; font-size: 1.02rem; color: {TEXT_MAIN}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
.focus-sub {{ color: {TEXT_SUB}; font-size: 0.78rem; margin-top: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
.focus-price {{ font-weight: 850; font-size: 1.18rem; text-align: right; color: {TEXT_MAIN}; }}
.focus-stat {{ color: {TEXT_SUB}; font-size: 0.78rem; }}
#MainMenu, #GithubIcon {{ visibility: hidden; }}
[data-testid="stToolbar"], [data-testid="stDecoration"], [data-testid="stStatusWidget"] {{ display: none !important; }}
footer {{ visibility: hidden; }}
[data-testid="collapsedControl"] {{ display: flex !important; visibility: visible !important; opacity: 1 !important; position: fixed !important; top: 75px !important; left: 12px !important; z-index: 999999 !important; width: 42px !important; height: 42px !important; border-radius: 50% !important; background: rgba(15,23,42,0.95) !important; border: 1px solid #38bdf8 !important; justify-content: center !important; align-items: center !important; box-shadow: 0 0 16px rgba(56,189,248,0.4); }}
[data-testid="collapsedControl"] svg {{ display: none !important; }}
[data-testid="collapsedControl"]::before {{ content: "⚙️"; font-size: 22px; }}
</style>
""",
...
refresh_seconds = st.sidebar.selectbox("갱신 주기", [15, 30, 60, 120, 300], index=2)
...
st.sidebar.divider()
custom_symbol = st.sidebar.text_input("🔎 빠른 티커 추가", placeholder="예: NVDA, QQQ, SOXL, PLTR, BTC-USD").upper().strip()
st.sidebar.divider()
chart_period = st.sidebar.selectbox("차트 기간", ["1d", "5d", "1mo", "3mo", "6mo", "1y"], index=0)
chart_interval = st.sidebar.selectbox("차트 간격", ["1m", "5m", "15m", "30m", "1h", "1d"], index=1)
st.sidebar.divider()
...
watchlist_assets = {
    ...
    "Meta": "META", "Tesla": "TSLA", "Alphabet": "GOOGL", "AMD": "AMD",
    "Broadcom": "AVGO", "Netflix": "NFLX", "Palantir": "PLTR", "TSMC": "TSM",
    "ARM": "ARM", "SOXL": "SOXL", "TQQQ": "TQQQ",
}
...
def search_yahoo_symbols(query):
    ...
    params = {"q": normalized_query, "quotesCount": 10, "newsCount": 0, "lang": "en-US", "region": "US"}
    ...
    try:
        ...
        for q in quotes:
            ...
            if quote_type in ["EQUITY", "ETF"] and symbol:
                results.append({"symbol": symbol, "name": name, "exchange": exchange, "type": quote_type})
        if not results and normalized_query.upper():
            results.append({"symbol": normalized_query.upper(), "name": normalized_query.upper(), "exchange": "직접 입력", "type": "UNKNOWN"})
        return results
    except Exception:
        return []
...
def get_stock_specific_news(symbol):
    try:
        ...
        feed = feedparser.parse(rss_url)
        return [{"title": entry.title, "link": entry.link, "time": entry.get("published", "")} for entry in feed.entries[:5]]
    except Exception:
        return []
...
def analyze_searched_stock(stock_data, stock_news, qqq_change, vix_price, tnx_change, dxy_change, smh_change):
    ...
    if any(word.lower() in f"{sector} {industry}".lower() for word in tech_words) and smh_change is not None and smh_change > 1:
...
with header_right:
    ...
    <html><head><style>
        ...
        .ticker-wrap {{ width: 100%; min-height: 126px; background: {TICKER_BG}; border: 1px solid {BORDER}; border-radius: 18px; padding: 12px 14px; box-sizing: border-box; overflow: hidden; }}
        ...
        .ticker-col {{ min-width: 0; }}
        .ticker-title {{ font-size: 12px; color: {TEXT_SUB}; margin-bottom: 6px; font-weight: 800; }}
        .ticker-window {{ height: 24px; overflow: hidden; position: relative; border-top: 1px solid {BORDER}; padding-top: 6px; }}
        ...
        @keyframes slideTicker {{
            0% {{ transform: translateY(0px); }} 22% {{ transform: translateY(0px); }}
            25% {{ transform: translateY(-24px); }} 47% {{ transform: translateY(-24px); }}
            50% {{ transform: translateY(-48px); }} 72% {{ transform: translateY(-48px); }}
            75% {{ transform: translateY(-72px); }} 97% {{ transform: translateY(-72px); }} 100% {{ transform: translateY(0px); }}
        }}
    </style></head><body>
        <div class="ticker-wrap">
            <div class="top-row"><div class="status-pill">{market_emoji} {market_status}</div><div class="time">{get_kst_now().strftime('%H:%M:%S')} KST</div></div>
            <div class="ticker-grid">
                <div class="ticker-col"><div class="ticker-title">📰 주요 뉴스</div><div class="ticker-window"><div class="ticker-track">
                    <div class="ticker-item">{safe_news_items[0]}</div><div class="ticker-item">{safe_news_items[1]}</div><div class="ticker-item">{safe_news_items[2]}</div><div class="ticker-item">{safe_news_items[3]}</div>
                </div></div></div>
                <div class="ticker-col"><div class="ticker-title">🧠 시장 코멘트</div><div class="ticker-window"><div class="ticker-track">
                    <div class="ticker-item">{safe_market_comments[0]}</div><div class="ticker-item">{safe_market_comments[1]}</div><div class="ticker-item">{safe_market_comments[2]}</div><div class="ticker-item">{safe_market_comments[3]}</div>
                </div></div></div>
            </div>
        </div>
    </body></html>
...
# =========================
top_metric_list = [
    ("💎 QQQ", "QQQ"), ("📈 나스닥", "^IXIC"), ("📊 S&P500", "^GSPC"),
    ("🧭 나스닥 선물", "NQ=F"), ("🧭 S&P 선물", "ES=F"),
    ("🛡️ VIX", "^VIX"), ("🏛️ 10년물 금리", "^TNX"), ("💵 DXY", "DX-Y.NYB"),
    ("🥇 금", "GC=F"), ("🛢️ 유가 WTI", "CL=F"), ("₿ 비트코인", "BTC-USD"),
    ("🔲 SMH", "SMH"), ("🧩 SOX", "^SOX"), ("🟩 NVDA", "NVDA"), ("🔴 AMD", "AMD"),
]
...
with left:
    ...
    if not chart_data.empty:
        fig = go.Figure()
        fig.add_trace(go.Candlestick(x=chart_data.index, open=chart_data["Open"], high=chart_data["High"], low=chart_data["Low"], close=chart_data["Close"], name=selected_symbol))
    ...
    if stock_search_query:
        ...
        if search_results:
            ...
            if stock_data:
                ...
                with s3:
                    st.metric("평균 대비 거래량", f"{stock_data['volume_ratio']}배" if stock_data["volume_ratio"] else "N/A")
                with s4:
                    st.metric("판단", signal)
                analysis_left, analysis_right = st.columns([1.4, 1])
                with analysis_left:
                    st.markdown(f"""
                    <div class="panel"><div class="comment-title">{stock_data['name']} ({selected_symbol_for_analysis})</div>
                    <div class="comment-desc">시가총액: {stock_data['market_cap']}<br>섹터: {stock_data['sector']}<br>산업: {stock_data['industry']}<br>애널리스트 평가: {stock_data['recommendation']}<br>평균 목표가: {stock_data['target_price']}</div><br>
                    <div class="comment-title">📌 종합 해석</div><div class="comment-desc">{comment}</div><div class="comment-signal">점수: {score} / 신호: {signal}</div></div>
                    """, unsafe_allow_html=True)
                with analysis_right:
                    reason_html = "".join([f"<div style='border-bottom:1px solid {BORDER}; padding:8px 0;'>• {reason}</div>" for reason in reasons])
                ...
                if stock_news:
                    ...
                    for idx, news in enumerate(stock_news[:4]):
                        with news_cols_for_stock[idx % 2]:
                            st.markdown(f"""
                            <div class="news-card"><div class="news-title"><a href="{news['link']}" target="_blank">{news['title']}</a></div><div class="news-link">뉴스 보기 →</div><div class="news-meta">{news['time']}</div></div>
                            """, unsafe_allow_html=True)
                else:
...
with middle:
    ...
    st.markdown(f"""
    <div class="panel">
        <div style="display:flex; justify-content:space-between; border-bottom:1px solid {BORDER}; padding-bottom:12px;"><span>종합 판단</span><span class="badge-green">{market_emoji} {market_status}</span></div>
    ...
    st.markdown(f"<div class='panel'><div style='white-space:pre-line; font-size:0.86rem; color:{TEXT_SUB};'>{economic_memo}</div></div>", unsafe_allow_html=True)
    ...
    for idx, symbol in enumerate(list(st.session_state.focus_symbols)):
        ...
        with col:
            if data:
                ...
                st.markdown(f"""
                <div class="focus-card"><div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start;"><div style="min-width:0; flex:1;"><div class="focus-name">{data['name']} ({symbol})</div><div class="focus-sub">{data['sector']} · {data['industry']}</div></div><div style="min-width:90px; text-align:right;"><div class="focus-price">{data['price']}</div><div class="{change_class}">{data['change']}%</div></div></div><div style="margin-top:13px; display:grid; grid-template-columns: 1fr 1fr; gap:8px;"><div class="focus-stat">거래량<br><b>{data['volume']:,}</b></div><div class="focus-stat" style="text-align:right;">평균대비<br><b>{volume_ratio_text}</b></div></div></div>
                """, unsafe_allow_html=True)
