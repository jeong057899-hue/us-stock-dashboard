...
if theme_mode == "라이트 모드":
    ...
    PANEL_BG = "linear-gradient(145deg, rgba(255,255,255,0.97), rgba(241,245,249,0.97))"
    TEXT_MAIN = "#0f172a"
    TEXT_SUB = "#475569"
    BORDER = "#cbd5e1"
    INPUT_BG = "#ffffff"
    PLOT_BG = "rgba(248,250,252,0.96)"
    TICKER_BG = PANEL_BG
...
[data-testid="stMetricValue"] {{ color: {TEXT_MAIN}; font-size: 1.45rem; font-weight: 800; }}
...
.comment-title {{ font-weight: 850; font-size: 0.92rem; color: {TEXT_MAIN}; }}
.comment-desc {{ font-size: 0.82rem; color: {TEXT_SUB}; line-height: 1.45; margin-top: 5px; }}
...
.news-title {{ font-size: 0.84rem; line-height: 1.35; font-weight: 750; }}
...
.news-link {{ font-size: 0.78rem; color: #38bdf8; margin-top: 8px; }}
.news-meta {{ font-size: 0.70rem; color: {TEXT_SUB}; margin-top: 5px; }}
...
.focus-price {{ font-weight: 850; font-size: 1.02rem; text-align: right; color: {TEXT_MAIN}; }}
.focus-stat {{ color: {TEXT_SUB}; font-size: 0.68rem; }}
.focus-remove button {{ height: 28px !important; padding: 2px 6px !important; font-size: 0.72rem !important; }}
@media (max-width: 1600px) {{ .focus-grid {{ grid-template-columns: repeat(3, minmax(0, 1fr)); }} }}
...
watchlist_assets = {
    ...
    "Tesla": "TSLA", "Alphabet": "GOOGL", "AMD": "AMD", "Broadcom": "AVGO", "Netflix": "NFLX",
    "Palantir": "PLTR", "TSMC": "TSM", "ARM": "ARM", "SOXL": "SOXL", "TQQQ": "TQQQ",
}
    ...
    for name, symbol in ticker_dict.items():
        try:
            ...
            if len(hist) >= 2:
                ...
                rows.append({
                    "이름": name, "티커": symbol, "현재가": round(current_price, 2),
...
if risk_score >= 3:
    market_status, market_emoji, market_comment = "Risk On", "🟢", "성장주와 위험자산에 우호적인 환경입니다."
elif risk_score <= -2:
    market_status, market_emoji, market_comment = "Risk Off", "🔴", "변동성, 금리, 달러 강세 등을 경계해야 합니다."
else:
    market_status, market_emoji, market_comment = "중립", "🟡", "방향성이 뚜렷하지 않아 주요 지표 확인이 필요합니다."
...
with header_right:
    ...
    @keyframes slideTicker {{
        ...
        75% {{ transform: translateY(-72px); }} 97% {{ transform: translateY(-72px); }}
...
if st.session_state.focus_symbols:
                ...
                <div class="focus-card">
                    <div style="display:flex; justify-content:space-between; gap:8px; align-items:flex-start;">
                        ...
                        <div style="min-width:70px; text-align:right;">
                            ...
                            <div class="{change_class}" style="font-size:0.78rem;">{data['change']}%</div>
                        </div>
                    </div>
    ...
    remove_target = st.selectbox("삭제할 보유/관심종목 선택", st.session_state.focus_symbols, key="focus_remove_select")
    if st.button("선택 종목 삭제", use_container_width=True):
        if remove_target in st.session_state.focus_symbols:
            st.session_state.focus_symbols.remove(remove_target)
            st.rerun()
else:
